...
import { DeleteProject, GetProjectDetails, ChangeProjectStatus } from '../../API/projectsAPI.js';
import { CreateProjectRequest, GetUserProjectGroup } from '../../API/managementAPI.js';
...
import AddUserIcon from '../../components/icons/AddUserIcon.jsx';
import RemoveUserIcon from '../../components/icons/RemoveUserIcon.jsx';
...
const ProjectDetails = () => {
    ...
    const canEditProjectInfo = user_id === projectInfo.created_by;
    ...
    const GetCurrentUserInfo = async (project_id, project_name) => {
        try {
            ...
            alert('Ошибка при загрузке данных пользователя, попробуйте позже.');
        }
    };
        ...
        <div className="detail-container">
            ...
            { projectInfo.is_gittable ? (
                <p>
                    ...
                    <a href={projectInfo.git_url} target="_blank" rel="noopener noreferrer">{projectInfo.git_url}</a>
                </p>
            ) : (
                <p><strong>GitFlic не подключен.</strong></p>
            )}
            { (isParticipant || is_admin) ? (
                <Button onClick={ToProject}>Перейти к задачам</Button>
            ) : (
