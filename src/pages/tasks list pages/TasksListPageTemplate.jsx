...
const TasksListPageTemplate = ({ id, GetTasksByProjectId, page_title, showCreator = true, showAssignee = true }) => {
    ...
    // Все доступные варианты для фильтрации:
    ...
    }, [id, navigate, GetTasksByProjectId]);
    ...
    .filter(task => !searchedObject || task.title.toLowerCase().includes(searchedObject.toLowerCase())); // фильтр по поисковому запросу
    ...
    return(
        <div className="page-container-for-sidebar">
            ...
            <div className="page-content">
                ...
                { createFormVisible && (
                    <CreateTaskForm createFormHidden={() => setCreateFormVisible(false)} 
                    projectId = {id}/>
                )}
                { tasksExistence? (<div className="tasks-grid">
                        {sortedTasks.map((task) => (
                            <div key={task.id} className="task-card">
                                <Link className='link' to={`/tasks/${task.id}/details`} 
                                    state={{ from: window.location.pathname, projectId: id }}>
                                    {task.title}
                                </Link>
                                <p>Дата сдачи: {new Date(task.due_date).toLocaleDateString()}<br/>
                ...
                </div>) : (<div className="tasks-grid">Похоже, заданий еще нет.</div>)}
            </div>
        </div>
    )
}
